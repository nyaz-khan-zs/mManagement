...
import { RoutePaths } from '../../utils/enum/route-path';
...
export class HttpService {
  ...
  createHeader(token?: string): HttpHeaders {
    ...
    } else return new HttpHeaders({ Authorization: `Bearer ${token}` });
  }

  refreshToken(): Observable<SessionData> {
    if (
      this.sessionData.userData.refreshToken === null ||
      this.sessionData.userData.refreshToken === undefined
    ) {
          ...
          return this.refreshToken().pipe(
            switchMap((refreshResponse) => {
              return this.httpClient.get(url, {
                headers: this.createHeader(refreshResponse.token),
              });
            }),
  ...
  post<T>(
    ...
    if (headers) {
      ...
      return this.httpClient
        .post(url, payload, { headers: this.createHeader() })
        .pipe(
          catchError((error) => {
            if (error.status === 401) {
              return this.refreshToken().pipe(
  ...
  put<T>(endPoint: string, payload: T): Observable<any> {
    return this.httpClient
      .put(`${this.API_URL}/${endPoint}`, payload, {
  ...
  delete(endPoint: string): Observable<any> {
    return this.httpClient
      .delete(`${this.API_URL}/${endPoint}`, { headers: this.createHeader() })
      .pipe(
        catchError((error) => {
          if (error.status === 401) {
            return this.refreshToken().pipe(

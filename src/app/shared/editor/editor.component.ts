...
import { Constants } from '../../utils/enum/constants.enum';
...
export class EditorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.htmlContent =
      this.selectedUpdate.update === null
        ? ''
        : this.selectedUpdate.update.remarks;
  }
  ...
  updateSelectedProjectHtmlContent(): void {
    if (this.htmlContent.trim() === '') return;
    for (let update of this.updates) {
      ...
      if (
        update.project !== null &&
        this.selectedUpdate.project !== null &&
        update.project.id === this.selectedUpdate.project.id
      ) {
        if (update.update === null) {
          update.update = {
            id: null,
  ...
  addUpdate(): void {
    ...
    if (
      this.selectedUpdate.update !== null &&
      this.selectedUpdate.update.remarks !== null &&
      this.selectedUpdate.update.id !== null
    ) {
      ...
      if (this.selectedUpdate.update.id !== null) {
        let postUpdate: UpdateRequest = {
          projectId:
            this.selectedUpdate.project !== null
              ? this.selectedUpdate.project.id
              : null,
